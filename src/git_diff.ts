...
import type { PrData, ChangedLine } from './types.js';

export function getDiffWithLineNumbers(baseBranch: string): Promise<PrData> {
        ...
        for (const string of matches) {
          if (string.includes(',')) {
            ...
            if (!isNaN(Number(iterations))) {
              ...
              const dataToConcat: ChangedLine = { lineNumber: number, endsAfter: iterations, line: [] };
